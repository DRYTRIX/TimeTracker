...
class InvoiceApprovalService:
    ...
    def request_approval(
        ...
        try:
            ...
            existing = InvoiceApproval.query.filter_by(
            ...
            if existing:
                return {
                    'success': False,
                    'message': 'An approval request is already pending for this invoice.'
                }
            ...
            if not safe_commit('request_approval', {'invoice_id': invoice_id}):
                return {
                    'success': False,
                    'message': 'Could not create approval request due to a database error.'
                }
            
            emit_event(WebhookEvent.INVOICE_APPROVAL_REQUESTED, {
                'invoice_id': invoice_id,
            ...
            return {
                'success': False,
                'message': f'Error creating approval request: {str(e)}'
            }
    ...
    def approve(
        ...
        try:
            ...
            if not safe_commit('approve_invoice', {'approval_id': approval_id}):
                return {
                    'success': False,
                    'message': 'Could not update approval due to a database error.'
                }
            
            # Emit event if fully approved
            if approval.status == 'approved':
                emit_event(WebhookEvent.INVOICE_APPROVED, {
            ...
            return {
                'success': False,
                'message': f'Error approving invoice: {str(e)}'
            }
    ...
    def reject(
        ...
        try:
            ...
            emit_event(WebhookEvent.INVOICE_REJECTED, {
                'invoice_id': approval.invoice_id,
                'approval_id': approval.id,
            ...
            return {
                'success': False,
                'message': f'Error rejecting invoice: {str(e)}'
            }
    ...
    def get_invoice_approval(self, invoice_id: int) -> Optional[InvoiceApproval]:
        ...
        return InvoiceApproval.query.filter_by(
            invoice_id=invoice_id
        ).order_by(InvoiceApproval.created_at.desc()).first()
