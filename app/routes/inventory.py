...
from app.models import (
    ...
    ProjectStockAllocation, Project, Supplier, SupplierStockItem,
...
def list_stock_items():
    ...
    return render_template('inventory/stock_items/list.html',
                         items=items,
                         search=search,
                         category=category,
                ...
                if supplier_id and supplier_id.strip():
                    try:
                        supplier_stock_item = SupplierStockItem(
                            ...
                            supplier_sku=supplier_skus[i].strip() if i < len(supplier_skus) and supplier_skus[i] else None,
                            unit_cost=Decimal(supplier_unit_costs[i]) if i < len(supplier_unit_costs) and supplier_unit_costs[i] else None,
                            minimum_order_quantity=Decimal(supplier_moqs[i]) if i < len(supplier_moqs) and supplier_moqs[i] else None,
                            lead_time_days=int(supplier_lead_times[i]) if i < len(supplier_lead_times) and supplier_lead_times[i] else None,
...
def view_stock_item(item_id):
    ...
    recent_movements = StockMovement.query.filter_by(stock_item_id=item_id)\
    ...
    return render_template('inventory/stock_items/view.html',
                         item=item,
...
def edit_stock_item(item_id):
    ...
    if request.method == 'POST':
        try:
            ...
            item.default_price = Decimal(request.form.get('default_price')) if request.form.get('default_price') else None
            ...
            item.reorder_point = Decimal(request.form.get('reorder_point')) if request.form.get('reorder_point') else None
            item.reorder_quantity = Decimal(request.form.get('reorder_quantity')) if request.form.get('reorder_quantity') else None
            ...
            existing_supplier_items = {si.id: si for si in SupplierStockItem.query.filter_by(stock_item_id=item.id).all()}
            processed_ids = set()
            
            for i, supplier_id in enumerate(supplier_ids):
                if supplier_id and supplier_id.strip():
                    try:
                        supplier_item_id = supplier_item_ids[i] if i < len(supplier_item_ids) and supplier_item_ids[i] else None
                        ...
                        if supplier_item_id and supplier_item_id.strip():
                            ...
                            if supplier_item_id_int in existing_supplier_items:
                                ...
                                supplier_item.supplier_sku = supplier_skus[i].strip() if i < len(supplier_skus) and supplier_skus[i] else None
                                supplier_item.unit_cost = Decimal(supplier_unit_costs[i]) if i < len(supplier_unit_costs) and supplier_unit_costs[i] else None
                                supplier_item.minimum_order_quantity = Decimal(supplier_moqs[i]) if i < len(supplier_moqs) and supplier_moqs[i] else None
                                supplier_item.lead_time_days = int(supplier_lead_times[i]) if i < len(supplier_lead_times) and supplier_lead_times[i] else None
                                supplier_item.is_preferred = supplier_item_id in supplier_preferred if supplier_preferred else False
...
def list_warehouses():
    ...
    return render_template('inventory/warehouses/list.html',
...
def view_warehouse(warehouse_id):
    ...
    stock_levels = WarehouseStock.query.filter_by(warehouse_id=warehouse_id)\
        .join(StockItem).order_by(StockItem.name).all()
    
    # Get recent movements
    recent_movements = StockMovement.query.filter_by(warehouse_id=warehouse_id)\
        .order_by(StockMovement.moved_at.desc()).limit(20).all()
    
    return render_template('inventory/warehouses/view.html',
                         warehouse=warehouse,
                         stock_levels=stock_levels,
                         recent_movements=recent_movements)
...
def stock_levels():
    ...
    if low_stock_only:
        stock_levels = [sl for sl in stock_levels if sl.stock_item.reorder_point and sl.quantity_on_hand < sl.stock_item.reorder_point]
    ...
    return render_template('inventory/stock_levels/list.html',
                         stock_levels=stock_levels,
                         warehouses=warehouses,
                         categories=categories,
...
def stock_levels_by_warehouse(warehouse_id):
    ...
    return render_template('inventory/stock_levels/warehouse.html',
                         warehouse=warehouse,
                         stock_levels=stock_levels,
                         categories=categories,
                         selected_category=category,
...
def stock_levels_by_item(item_id):
    ...
    return render_template('inventory/stock_levels/item.html',
                         item=item,
                         stock_levels=stock_levels)
...
def list_movements():
    ...
    return render_template('inventory/movements/list.html',
                         movements=movements,
...
def new_movement():
    ...
    if request.method == 'POST':
        try:
            ...
            log_event('stock_movement_created', {
    ...
    return render_template('inventory/movements/form.html',
...
def list_transfers():
    ...
    return render_template('inventory/transfers/list.html',
    ...
    if request.method == 'POST':
        try:
            ...
            if from_warehouse_id == to_warehouse_id:
                ...
                stock_items = StockItem.query.filter_by(is_active=True, is_trackable=True).order_by(StockItem.name).all()
                warehouses = Warehouse.query.filter_by(is_active=True).order_by(Warehouse.code).all()
                return render_template('inventory/transfers/form.html',
                                     stock_items=stock_items,
                                     warehouses=warehouses)
            ...
            log_event('stock_transfer_created', {
...
def list_adjustments():
    ...
    return render_template('inventory/adjustments/list.html',
...
def new_adjustment():
    ...
    if request.method == 'POST':
        try:
            ...
            log_event('stock_adjustment_created', {
    ...
    return render_template('inventory/adjustments/form.html',
                         stock_items=stock_items,
                         warehouses=warehouses)
...
def stock_item_history(item_id):
    ...
    return render_template('inventory/stock_items/history.html',
                         item=item,
                         movements=movements,
                         warehouses=warehouses,
                         selected_warehouse_id=warehouse_id,
...
def list_reservations():
    ...
    return render_template('inventory/reservations/list.html',
...
def view_supplier(supplier_id):
    ...
    supplier_items = SupplierStockItem.query.options(
        joinedload(SupplierStockItem.stock_item)
    ).filter_by(
    ...
    return render_template('inventory/suppliers/view.html',
                         supplier=supplier,
...
def list_purchase_orders():
    ...
    return render_template('inventory/purchase_orders/list.html',
...
def new_purchase_order():
    ...
    if request.method == 'POST':
        try:
            ...
            for i, desc in enumerate(item_descriptions):
                if desc.strip():
                    try:
                        stock_item_id = int(item_stock_ids[i]) if i < len(item_stock_ids) and item_stock_ids[i] else None
                        supplier_stock_item_id = int(item_supplier_stock_ids[i]) if i < len(item_supplier_stock_ids) and item_supplier_stock_ids[i] else None
                        warehouse_id = int(item_warehouse_ids[i]) if i < len(item_warehouse_ids) and item_warehouse_ids[i] else None
                        
                        item = PurchaseOrderItem(
                            purchase_order_id=purchase_order.id,
                            description=desc.strip(),
                            quantity_ordered=Decimal(item_quantities[i]) if i < len(item_quantities) and item_quantities[i] else Decimal('1'),
                            unit_cost=Decimal(item_unit_costs[i]) if i < len(item_unit_costs) and item_unit_costs[i] else Decimal('0'),
                            stock_item_id=stock_item_id,
                            supplier_stock_item_id=supplier_stock_item_id,
                            supplier_sku=item_supplier_skus[i].strip() if i < len(item_supplier_skus) and item_supplier_skus[i] else None,
                            warehouse_id=warehouse_id,
            ...
            log_event('purchase_order_created', {'purchase_order_id': purchase_order.id, 'po_number': purchase_order.po_number})
    ...
    return render_template('inventory/purchase_orders/form.html',
                         purchase_order=None,
                         suppliers=suppliers,
                         warehouses=warehouses,
                         stock_items=stock_items)
...
def view_purchase_order(po_id):
    ...
    return render_template('inventory/purchase_orders/view.html',
...
def edit_purchase_order(po_id):
    ...
    if request.method == 'POST':
        try:
            ...
            purchase_order.expected_delivery_date = datetime.strptime(request.form.get('expected_delivery_date'), '%Y-%m-%d').date() if request.form.get('expected_delivery_date') else None
...
def receive_purchase_order(po_id):
    ...
    if request.method == 'POST':
        try:
            ...
            received_date = datetime.strptime(received_date_str, '%Y-%m-%d').date() if received_date_str else datetime.utcnow().date()
...
def reports_dashboard():
    ...
    total_value = db.session.query(
        func.sum(WarehouseStock.quantity_on_hand * StockItem.default_cost)
    ).join(StockItem).filter(StockItem.default_cost.isnot(None)).scalar() or 0
...
def reports_movement_history():
    ...
    return render_template('inventory/reports/movement_history.html',
                         movements=movements,
                         warehouses=warehouses,
                         stock_items=stock_items,
                         selected_warehouse_id=warehouse_id,
...
def reports_turnover():
    ...
    ).group_by(StockItem.id).all()
    
    turnover_data = []
    for item, total_sold in items_with_sales:
        avg_stock = db.session.query(
            func.avg(WarehouseStock.quantity_on_hand)
        ).filter_by(stock_item_id=item.id).scalar() or 0
        
        days_in_period = (date_to_obj - date_from_obj).days
        turnover_rate = 0
        if avg_stock > 0:
            turnover_rate = abs(float(total_sold or 0)) / float(avg_stock) * (365 / days_in_period) if days_in_period > 0 else 0
    ...
    return render_template('inventory/reports/turnover.html',
...
def reports_low_stock():
    ...
    return render_template('inventory/reports/low_stock.html',
