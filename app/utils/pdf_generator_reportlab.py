...
class ReportLabTemplateRenderer:
    ...
    def _on_page(self, canv: canvas.Canvas, doc: SimpleDocTemplate):
        ...
        for element_data in getattr(self, 'absolute_elements', []):
            ...
            # Issue #537: IMAGE elements (especially decorative) - don't skip when overflowing;
    ...
    def _draw_image_on_canvas(self, canv: canvas.Canvas, element: Dict[str, Any], x: float, y: float, page_height: float):
        ...
        # Issue #537: Draw images at full size when overflowing; PDF clips at page boundary
        # so the visible portion will show in the exported PDF
        if width <= 0 or height <= 0:
            return
        ...
        try:
            ...
            if source.startswith("data:image"):
                ...
                try:
                    img_data = base64.b64decode(parts[1])
                    img_reader = ImageReader(io.BytesIO(img_data))
                    canv.drawImage(img_reader, x, y - height, width=width, height=height, preserveAspectRatio=preserve_aspect, mask='auto')
                except Exception as e:
                    if current_app:
                        current_app.logger.error(f"Error decoding base64 image for canvas: {e}")
                    return
            # Handle template image URLs (convert to file path or base64)
            # Issue #537: Also handle full URLs (https://host/uploads/template_images/xxx)
                ...
                try:
                    ...
                    if base64_data:
                        ...
                        file_path = self._resolve_template_image_path(filename)
                        if file_path and os.path.exists(file_path):
                            canv.drawImage(file_path, x, y - height, width=width, height=height, preserveAspectRatio=preserve_aspect, mask='auto')
                        elif current_app:
                            current_app.logger.warning(f"Template image not found: {source} (tried get_image_base64 and direct paths)")
                except Exception as e:
                    if current_app:
                        current_app.logger.error(f"Error loading template image {source}: {e}")
                    else:
                ...
                canv.drawImage(source, x, y - height, width=width, height=height, preserveAspectRatio=preserve_aspect, mask='auto')
        except Exception as e:
            if current_app:
